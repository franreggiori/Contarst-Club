...
export const locationSchedules: LocationSchedule[] = locations.map((loc) => ({
  ...
  openHours: loc.hours,
  guidedTimes: ['07:00', '08:00', '18:00', '19:00'],
...
export const slots: Slot[] = upcomingDates.flatMap((date, i) =>
  locations.flatMap((loc, li) => {
    const guided = [
      ...
      { time: '08:00', serviceId: 'fundamentals', tags: ['Beginner friendly'] },
      { time: '18:00', serviceId: 'performance-reset', tags: ['High intensity'] },
      { time: '19:00', serviceId: i % 5 === 0 ? 'night-ritual' : 'ritual-guiado', tags: ['Group energy'] }
    ];
    ...
    return [...guided, ...open].map((item, si) => ({
      id: `${loc.id}-${date}-${item.time}-${item.serviceId}`,
      date,
      time: item.time,
      serviceId: item.serviceId,
      locationId: loc.id,
      capacity: item.serviceId === 'open-contrast' ? 20 : 14,
      spotsLeft: Math.max(1, (item.serviceId === 'open-contrast' ? 20 : 14) - ((i + si + li) % 11)),
